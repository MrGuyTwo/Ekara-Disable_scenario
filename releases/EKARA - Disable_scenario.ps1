...
$global:UserName = "xxxxxxxxxxxxxxx"                                                            # EKARA Account
...
Function List_Scenario_ID{
    try{
        ...
        Write-Host "------------------- Liste les scenarios désactivés -------------------" -BackgroundColor "White" -ForegroundColor "DarkCyan"
        $uri ="$API/adm-api/scenarios"
        $scenarios = Invoke-RestMethod -Uri $uri -Method POST -Headers $headers -Verbose 
        $scenarios = $scenarios | Where-Object {$_.active -EQ 1}                                             #List only Disabled scenarios
        ...
        $form.Text = 'List of active scenarios'
        ...
        $label.Text = 'Select the scenarios to deactivate:'
        $label.AutoSize = $true
        ...
        if (($result -eq [System.Windows.Forms.DialogResult]::OK) -and $listbox.SelectedItems.Count -gt 0)
        {
            ...
            Write-Host ("Sénario disabled : " + $Result_OK)
            [System.Windows.Forms.MessageBox]::Show("Sénario disabled : $Result_OK","Resultat",[System.Windows.Forms.MessageBoxButtons]::OKCancel,[System.Windows.Forms.MessageBoxIcon]::Information)
...
"@
         $uri ="$API/adm-api/scenario/$ScenatioID/stop"
         $requestWS = Invoke-RestMethod -Uri $uri -Method GET -Headers $headers -Verbose
         if ($requestWS.success -eq $true)   {
            Write-Host "Sécénario [$ScenatioName] désactivé avec sucess"
            Write-Host $requestWS.message
            $global:Result_OK = $global:Result_OK+1
         }else{
            Write-Host "Echec lors de la désactivation du scénario [$ScenatioName]"
